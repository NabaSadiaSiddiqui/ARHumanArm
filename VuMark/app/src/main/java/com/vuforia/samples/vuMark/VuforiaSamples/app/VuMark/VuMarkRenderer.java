...
import com.vuforia.samples.vuMark.SampleApplication.SampleAppRenderer;
import com.vuforia.samples.vuMark.SampleApplication.SampleAppRendererControl;
import com.vuforia.samples.vuMark.SampleApplication.SampleApplicationSession;
import com.vuforia.samples.vuMark.SampleApplication.utils.CubeShaders;
import com.vuforia.samples.vuMark.SampleApplication.utils.SampleUtils;
...
public class VuMarkRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    ...
    private float vumarkHeight = 0.0f;
    ...
    public void renderFrame(State state, float[] projectionMatrix)
    {
        ...
        int tIdx = 0;
        for (tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
        {
            TrackableResult result = state.getTrackableResult(tIdx);
        ...
        if(gotVuMark)
        {
            ...
            if(state.getNumTrackableResults() == 2) {
                TrackableResult wrist = state.getTrackableResult(0);
                TrackableResult elbow = state.getTrackableResult(1);

                Matrix44F modelViewMatrix_VuforiaWrist = Tool.convertPose2GLMatrix(wrist.getPose());
                Matrix44F modelViewMatrix_VuforiaElbow = Tool.convertPose2GLMatrix(elbow.getPose());

                float[] modelViewMatrixWrist = modelViewMatrix_VuforiaWrist.getData();
                float[] modelViewMatrixElbow = modelViewMatrix_VuforiaElbow.getData();
                float[] modelViewMatrix = new float[modelViewMatrixWrist.length];
                for(int i=0; i<modelViewMatrix.length; i++) {
                    modelViewMatrix[i] = modelViewMatrixWrist[i] + modelViewMatrixElbow[i];
                }

                if (wrist.isOfType(VuMarkTargetResult.getClassType()) && elbow.isOfType(VuMarkTargetResult.getClassType())) {
                    VuMarkTargetResult vmtResultWrist = (VuMarkTargetResult) wrist;
                    VuMarkTargetResult vmtResultElbow = (VuMarkTargetResult) elbow;

                    VuMarkTarget vmTgtWrist = (VuMarkTarget) vmtResultWrist.getTrackable();
                    VuMarkTarget vmTgtElbow = (VuMarkTarget) vmtResultElbow.getTrackable();

                    VuMarkTemplate vmTmpWrist = (VuMarkTemplate) vmTgtWrist.getTemplate();
                    VuMarkTemplate vmTmpElbow = (VuMarkTemplate) vmTgtElbow.getTemplate();

                    boolean isMainVuMark = ((indexVuMarkToDisplay < 0) || (indexVuMarkToDisplay == tIdx));
                    ...
                    // Add a translation to recenter the augmentation
                    // on the VuMark center, w.r.t. the origin
                    Vec2F originWrist = vmTmpWrist.getOrigin();
                    Vec2F originElbow = vmTmpElbow.getOrigin();

                    float translXWrist = -originWrist.getData()[0];
                    float translYWrist = -originWrist.getData()[1];
                    float translXElbow = -originElbow.getData()[0];
                    float translYElbow = -originElbow.getData()[1];
                    float translX = (translXWrist + translXElbow)/2;
                    float translY = (translYWrist + translYElbow)/2;
                    ...
                    float[] modelViewMatrixWristInv = new float[16];
                    Matrix.invertM(modelViewMatrixWristInv, 0, modelViewMatrixWrist, 0);
                    float[] modelViewElbowToWrist = new float[16];
                    Matrix.multiplyMM(modelViewElbowToWrist, 0, modelViewMatrixElbow, 0, modelViewMatrixWristInv, 0);
                    vumarkHeight = modelViewElbowToWrist[6] < 0.1 && vumarkHeight < modelViewElbowToWrist[6] ? modelViewElbowToWrist[6] : vumarkHeight;

                    float vumarkWidth = vumarkWidthWrist > vumarkWidthElbow ? vumarkWidthWrist : vumarkWidthElbow;
                    Matrix.scaleM(modelViewMatrix, 0, vumarkWidth * VUMARK_SCALE, vumarkHeight * VUMARK_SCALE, 1.0f);
                    ...
                    GLES20.glVertexAttribPointer(vertexHandle, 3, GLES20.GL_FLOAT, false, 0, mPlaneObj.getVertices());
                    GLES20.glVertexAttribPointer(textureCoordHandle, 2, GLES20.GL_FLOAT, false, 0, mPlaneObj.getTexCoords());
                    ...
                    // finally draw the plane
                    GLES20.glDrawElements(GLES20.GL_TRIANGLES, mPlaneObj.getNumObjectIndex(), GLES20.GL_UNSIGNED_SHORT, mPlaneObj.getIndices());
